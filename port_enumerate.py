...
def read_from_port(ser, flag_index):
	global ser_read_thread_active_flag_list, ser_read_try_count
	...
	while ser_read_thread_active_flag_list[flag_index]:
		...
		try:
			ser_read_try_count +=1
			if ser_read_try_count > 20:
				print("Trying to reset Arduino at", ser.name)
				cmd = "ard-reset-arduino "+ser.name
				os.system(cmd)
